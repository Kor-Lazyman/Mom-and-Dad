...
def main():
    ...
    while(done==False):
        ...
        for k in range(10000):
            ...
            while not done:
                ...
                Manufacture_df,Present_Manufacturer_Inventory=Manufacturer_Env(week,Manufacture,Manufacture_df,Present_Manufacturer_Inventory,Distributer_df)
                Distributer_df,Present_Distributer_Inventory=Distributer_Env(week,Distributer,Distributer_df,Present_Distributer_Inventory,Wholesaler_df,Manufacture_df)
                Wholesaler_df,Present_Wholesaler_Inventory=Wholesaler_Env(week,Wholesaler,Wholesaler_df,Present_Wholesaler_Inventory,Retailer_df,Distributer_df)
                Retailer_df,Present_Retailer_Inventory,cost=Retailer_Env(week,Retailer,Retailer_df,Present_Retailer_Inventory,Wholesaler_df,action)
        
        
                x=week
                y=action
                reward=cost
                history.append((x,action,cost))
    ...
    for x in range(len(df)):
        for y in range(len(df.loc[1])):
            df.loc[x][y]=df.loc[x][y]/100000
            
    for x in range(len(df)):
        print(np.argmax(df.loc[x]))
